...
tarotDeck.forEach((card, index) => {
  ...
  if (index <= 21) {
    ...
    fileName = `maj${num}.jpg`;
  } else if (index <= 35) {
    let num = index - 21;
    num = num < 10 ? "0" + num : num;
    fileName = `wands${num}.jpg`;
  } else if (index <= 49) {
    let num = index - 35;
    num = num < 10 ? "0" + num : num;
    fileName = `cups${num}.jpg`;
  } else if (index <= 63) {
    let num = index - 49;
    num = num < 10 ? "0" + num : num;
    fileName = `swords${num}.jpg`;
  } else {
    let num = index - 63;
    num = num < 10 ? "0" + num : num;
    fileName = `pents${num}.jpg`; 
  }
...
function shuffleAndShowDeck() {
  ...
  deck.forEach((card, index) => {
    ...
    const baseTransform = `rotateY(${angle}deg) translateZ(${ringRadius}px)`;
    cardElem.dataset.baseTransform = baseTransform;
    cardElem.style.transform = `rotateY(${angle}deg) translateZ(0px) scale(0)`;
...
document.getElementById('drawSelected').addEventListener('click', () => {
  ...
  selectedCards.forEach((card) => {
    ...
    const renderCardContent = () => {
      ...
      div.innerHTML = `
        <img src="${card.image}" 
             onerror="this.src='https://via.placeholder.com/150x250/0d053d/ffd700?text=No+Image'" 
             style="${imgTransform}">
        <h3>${card.name}</h3>
        <small>${orient}</small>
        <p>${meaning}</p>
